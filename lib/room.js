...
class Room {
    ...
    appendMessage(self, channel_chat, m) {
        ...
        if(m.user_id == window.current_user_id) {
            ...
            // TODO If the message is an image or link, display only the link/src when you edit it
            raw_message = m.message_message;
            
            //raw_message.replace(raw_message, /<.*emotecode="(.*?)".*>/g);
            //raw_message.replace(raw_message, /<.*href="(.*?)".*>/g);
            //raw_message.replace(raw_message, /<.*src="(.*?)".*>/g);
            
            message_editor.setAttribute('value', raw_message);
                ...
                if(confirm("Are you sure you want to delete this message?")) {
                    ...
                    Ajax('PUT', '/room/' + self.room_data.room_id + '/message/' + m.message_id, ['user_id=' + m.user_id, 'message_message=' + edited_message, 'message_timestamp=' + edited_timestamp], function(r, c) {
                        ...
                        if(c == 201) {
                            ...
                            actual_message_span.innerHTML = '<em>' + new_message.message_message + '</em>';
                            sender_span.innerHTML = new_message.message_timestamp + ' | ' + new_message.message_sender_name;
