...
class Room {
    ...
    constructor(room, user_id) {
        ...
        var self = this,
            ...
            admin_control_button = document.createElement('span'),
            admin_control_img = document.createElement('img'),
            admin_control_window_open = false;
        ...
        // Set up admin control button if the user is an admin
        if(window.current_user_id == room.room_admin_id) {
            admin_control_button.setAttribute('class', 'admin-control-button');
            admin_control_img.setAttribute('src', 'img/admin_button.png');
            admin_control_img.setAttribute('title', 'Admin Controls');
            admin_control_button.appendChild(admin_control_img);

            admin_control_button.addEventListener('click', function(e) {
                // If the admin window isn't already open
                if(!admin_control_window_open) {
                    
                    admin_control_window_open = true;
                    
                    var i,
                        admin_control_window = document.createElement('div'),
                        title_span = document.createElement('p'),
                        title_text_node = document.createTextNode('Admin controls for #' + room.room_name),
                        user_list = document.createElement('ul');
                    
                    title_span.setAttribute('class', 'admin-control-title');
                    title_span.appendChild(title_text_node);
                    
                    admin_control_window.setAttribute('class', 'admin-control-window');
                    
                    admin_control_window.appendChild(title_span);
                    admin_control_window.appendChild(user_list);
                    
                    Ajax('GET', '/room/' + room.room_id + '/userstatus', [], function(r, c) {
                        
                        if(c == 200) {
                            
                            var user_statuses = get_json_objects_from_result(r)[0].user_statuses;
                            
                            // Loop through the users in this room and give controls to mute, kick and ban them
                            for(i = 0; i < user_statuses.length; i++) {

                                if(user_statuses[i].user_id == window.current_user_id) continue;

                                (function(user_status) {

                                    var user_full_name = document.createTextNode(user_status.user_first_name + ' ' + user_status.user_last_name),
                                        user_controls_container = document.createElement('li'),
                                        mute_text = document.createTextNode((user_status.user_status == 1) ? 'Unmute' : 'Mute'),
                                        kick_text = document.createTextNode('Kick'),
                                        ban_text = document.createTextNode((user_status.user_status == 3) ? 'Unban' : 'Ban'),
                                        pipe1 = document.createTextNode(' | '),
                                        pipe2 = document.createTextNode(' | '),
                                        pipe3 = document.createTextNode(' | '),
                                        mute = document.createElement('a'),
                                        kick = document.createElement('a'),
                                        ban = document.createElement('a');

                                    mute.setAttribute('href', '#');
                                    mute.setAttribute('status-action', 'mute');

                                    kick.setAttribute('href', '#');
                                    kick.setAttribute('status-action', 'kick');

                                    ban.setAttribute('href', '#');
                                    ban.setAttribute('status-action', 'ban');

                                    mute.appendChild(mute_text);
                                    kick.appendChild(kick_text);
                                    ban.appendChild(ban_text);
                                    
                                    user_controls_container.appendChild(user_full_name);
                                    user_controls_container.appendChild(pipe1);
                                    
                                    if(user_status.user_status != 3) {
                                        user_controls_container.appendChild(mute);
                                        user_controls_container.appendChild(pipe2);
                                        user_controls_container.appendChild(kick);
                                        user_controls_container.appendChild(pipe3);
                                    }
                                    
                                    user_controls_container.appendChild(ban);

                                    user_controls_container.addEventListener('click', function(e) {

                                        switch(e.target.getAttribute('status-action')) {
                                            case 'mute':

                                                user_status.user_status = (user_status.user_status == 1) ? 0 : 1;

                                                mute_text.nodeValue = (user_status.user_status == 1) ? 'Unmute' : 'Mute';
                                            ...
                                            case 'kick':
                                                ...
                                                Ajax('DELETE', '/user/' + user_status.user_id + '/room', ['room_id=' + room.room_id], function(r, c) {

                                                    if(c == 200) {
                                                        user_controls_container.innerHTML = user_full_name.nodeValue + ' was kicked from ' + room.room_name + '.';
                                                    }
                                                    else {
                                                        console.log(c + ' error when kicking user: ' + r);
                                                    }
                                            ...
                                            case 'ban':
                                                ...
                                                user_status.user_status = (user_status.user_status == 3) ? 0 : 3;

                                                ban_text.nodeValue = (user_status.user_status == 3) ? 'Unban' : 'Ban';
                                        ...
                                        if(user_status.user_status != 2) {
                                            ...
                                            Ajax('PUT', '/room/' + room.room_id + '/userstatus', ['user_id=' + user_status.user_id, 'user_status=' + user_status.user_status], function(r, c) {

                                                if(c == 201) {
                                                    console.log('Successfully changed user status.');
                                                }
                                                else {
                                                    console.log('Error when changing user status: ' + r);
                                                }
                                    ...
                                    user_list.appendChild(user_controls_container);
                            ...
                            console.log(c + ' error when getting user status: ' + r);
                        }
                        
                    });
                    
                    channel.appendChild(admin_control_window);
                }
                else {
                    admin_control_window_open = false;
                    channel.removeChild(channel.getElementsByClassName('admin-control-window')[0]);
                }
            });
        }
        ...
        gallery_view_img.setAttribute('title', 'View this room\'s gallery');
        ...
        channel.appendChild(admin_control_button);
        ...
        this.user_status = room.user_status;
    }
    ...
    initInput() {
        ...
        channel_textarea.setAttribute('roomid', this.room_data.room_id);
        ...
        channel_textarea.addEventListener('keydown', function(e) {
            if(e.keyCode == 13) { // The enter key
                ...
                // Check if the user trying to post is muted or banned
                var i,
                    current_user_status,
                    status_message = {
                        'message_message': '',
                        'message_timestamp': self.getCurrentTimestamp(),
                        'message_sender_name': 'System'
                    };
                
                Ajax('GET', '/room/' + self.room_id + '/userstatus', [], function(r, c) {
                    ...
                    if(c == 200) {
                        ...
                        for(i = 0; i < user_statuses.length; i++) {
                            if(window.current_user_id == user_statuses[i].user_id) {
                                current_user_status = user_statuses[i].user_status;
                                break;
                            }
                        }
                        ...
                        if(current_user_status == 1) {
                            status_message.message_message = 'You are muted in this channel and are unable to send messages.';
                            self.appendMessage(self, status_message);
                            return false;
                        }
                        // If the user is banned
                        else if(current_user_status == 3) {
                            status_message.message_message = 'You are banned from this channel.';
                            self.appendMessage(self, status_message);
                            return false;
                        }
                        ...
                        */
                        ...
                        console.log('Failed to append message: ' + c + ' error when getting user status: ' + r);
                    }
    ...
    appendMessage(self, m) {
        ...
        message_span.setAttribute('raw-content', m.message_raw_content);
    ...
    Ajax('GET', '/user/' + window.current_user_id + '/room', [], function(r, c) {
        ...
        if(c == 200) {
            ...
            for(i = 0; i < rooms.length; i++) {
                var new_room = new Room(rooms[i], window.current_user_id);
                window.chatapp.joined_rooms.push(new_room);
            }
