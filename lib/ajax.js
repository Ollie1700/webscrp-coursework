function Ajax(verb, resource, params, callback) {
    ...
    if (params !== null) {
        for (var i = 0; i < params.length; i++) {
            var 
                keyValue = params[i].split(/=(.+)/), // This regex splits the string at the 1st equals only. This prevents unexpected behaviour when the message itself contains =
                key = keyValue[0],
                value = keyValue[1];
            
            param_str += key + '=' + escape(value) + (i + 1 == params.length ? '' : '&');
        }
    }
