function Ajax(verb, resource, data, callback) {
    ...
    ajaxObj.onerror = function(e) {
        toggleDisconnectMessages(true);
    }
	...
	ajaxObj.onreadystatechange = function() {
        ...
        // If we were disconnected and have reconnected, remove the error messages
        if(disconnected) toggleDisconnectMessages(false);
...
function toggleDisconnectMessages(toggle) {
    var i,
        divs = document.getElementsByClassName('disconnect-message');
    
    disconnected = toggle;

    for(i = 0; i < divs.length; i++) {
        divs[i].style.display = toggle ? 'block' : 'none';
    }
}
